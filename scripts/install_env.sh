...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --env-name)
      if [[ $# -lt 2 ]]; then
        echo "--env-name requires a conda environment name" >&2
        exit 1
      fi
      FLUXVLA_CONDA_ENV_NAME="${2:-}"
      shift 2
      ;;
    --python-version)
      if [[ $# -lt 2 ]]; then
        echo "--python-version requires a value, e.g. 3.10" >&2
        exit 1
      fi
      FLUXVLA_PYTHON_VERSION="${2:-}"
      shift 2
      ;;
    --dry-run)
      DRY_RUN=1
...
ensure_conda_env() {
  ...
  conda_bin="$(find_conda_bin || true)"
  if [[ -z "${conda_bin}" ]]; then
    echo "Error: no active conda environment and no conda installation found." >&2
    echo "  Install Miniconda/Anaconda, or activate an existing env, then rerun." >&2
    exit 1
  fi

  local conda_root env_prefix conda_sh
  conda_root="$(cd "$(dirname "${conda_bin}")/.." && pwd)"
  env_prefix="${conda_root}/envs/${FLUXVLA_CONDA_ENV_NAME}"
  conda_sh="${conda_root}/etc/profile.d/conda.sh"

  if [[ -d "${env_prefix}" ]]; then
    echo "conda env '${FLUXVLA_CONDA_ENV_NAME}' already exists at ${env_prefix}; activating it."
  else
    echo "No active conda environment; creating '${FLUXVLA_CONDA_ENV_NAME}' (python=${FLUXVLA_PYTHON_VERSION})."
    run_conda_with_timeout "${conda_bin}" create -y -n "${FLUXVLA_CONDA_ENV_NAME}" "python=${FLUXVLA_PYTHON_VERSION}"
  fi
  ...
  if [[ ! -f "${conda_sh}" ]]; then
    echo "Error: could not find ${conda_sh} to activate the new environment." >&2
    exit 1
  fi
  set +u
  # shellcheck disable=SC1090
  source "${conda_sh}"
  conda activate "${FLUXVLA_CONDA_ENV_NAME}"
  set -u

  if [[ "${CONDA_PREFIX:-}" != "${env_prefix}" ]]; then
    echo "Error: failed to activate conda env '${FLUXVLA_CONDA_ENV_NAME}' after creating it." >&2
    exit 1
  fi
  PYTHON_BIN="${CONDA_PREFIX}/bin/python"
