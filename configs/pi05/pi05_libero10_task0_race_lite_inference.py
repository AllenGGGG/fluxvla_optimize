...
"""RACE-lite inference for full_v2 + Ultra-V2.

This is a deployment-side approximation of RACE ideas for the current delta
action policy: risk-scheduled chunk retiming plus chunk-continuity guarding.
It does not reproduce the full RACE training target or TOPP-RA pipeline.
"""
...
eval = dict(
    ...
    action_postprocess=dict(
        ...
        chunk_continuity_guard=True,
        continuity_error_ratio=1.75,
        continuity_abs_floor=1e-8,
        race_candidate_search=True,
        race_search_objective='fastest_feasible',
        race_exclude_uncompressed=False,
        race_compression_weight=1.0,
        race_path_mean_weight=8.0,
        race_path_max_weight=3.0,
        race_first_step_weight=0.5,
        race_continuity_weight=0.6,
        race_delta_weight=0.35,
        race_accel_weight=0.25,
        smooth_type='none',
        length_weight=1.0,
        accel_weight=0.35,
        turn_weight=0.35,
