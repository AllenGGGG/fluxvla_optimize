...
#
# fsdp_v0.1.4_recap branch's training config: pi05_parcel_sort.py's
# model/runner, with the episode-metadata prompt + window-level VSTA speed
# augmentation work from this branch wired in (VSTAPackedParquetDatasetV3,
# EpisodeMetadataPrompter, SelectJointDims). Currently points at a v3.0
# dataset produced by ros2_data_collection's dummy_writer_demo.py
# (--dataset-version v3.0) + scripts/compute_norm_stats.py, since real
# parcel_sort data has not been re-recorded in v3.0 yet -- swap
# data_root_path once it has been, and re-run compute_norm_stats.py (see the
# WARNING above SelectJointDims in `transforms=[...]` below).
        ...
        datasets=[
            dict(
                ...
                transforms=[
                    ...
                    # same values via `compute_norm_stats.py
